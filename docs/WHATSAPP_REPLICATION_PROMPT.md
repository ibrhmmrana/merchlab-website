...
Use this prompt in a **new Next.js (App Router) project** where you already have a Meta WhatsApp webhook that receives incoming messages and currently replies with "Received". The goal is to replicate the full foundation: **webhook with AI/human handover**, **Supabase storage**, **generic AI agent**, and **admin dashboard to monitor conversations and take over**.
