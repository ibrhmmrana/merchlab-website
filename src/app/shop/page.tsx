...
function ShopPageContent() {
  ...
  // Track catalog view once when product list is shown
  ...
  useEffect(() => {
    if (groups.length > 0 && !catalogTrackedRef.current) {
      ...
      import('@/lib/analytics/metaPixel').then(({ metaPixel }) => {
        metaPixel.viewContent({ content_type: 'product_catalog', content_category: 'shop' });
      });
    }
  }, [groups.length]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex flex-col md:flex-row gap-2 md:items-center md:justify-between mb-4">
        <form onSubmit={async (e)=>{e.preventDefault(); if (search.trim()) { const { metaPixel } = await import('@/lib/analytics/metaPixel'); metaPixel.search({ search_string: search.trim(), content_category: 'shop' }); } load(true);}} className="flex gap-2">
